...
- Gemini handles PDFs natively via base64 `inlineData`. An earlier OpenAI experiment was reverted because `gpt-4o` rejected PDF MIME types — keep the single-file Gemini path; don't reintroduce client-side PDF→image conversion.
...
- An earlier OpenAI/multi-file experiment was reverted in full. If you spot a fresh import of `openai`, `pdf-lib`, `pdfjs-dist`, or `canvas`, it was probably re-added accidentally — `googleapis` is the only one of those that's back deliberately (Phase 12 Drive). The code path is single-file Gemini only.
